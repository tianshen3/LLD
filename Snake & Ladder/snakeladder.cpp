...
//observer pattern for notififcation system
class IObserver {
    public:
...
//custom strategy user provides the count
class CustomCountBoardSetupStrategy : public BoardSetupStrategy {
    ...
    public:
    CustomCountBoardSetupStrategy(int snake, int ladders, bool random){
        numSnakes = snake;
        numLadders = ladders;
    ...
    void addSnakePosition(int start, int end){
        snakePositions.push_back({start, end});
    }

    void addLadderPositions(int start, int end){
        ladderPositions.push_back({start, end});
    }
    ...
    if(choice == 1) {
        ...
        if(customChoice == 1) {
            ...
            for(int i = 0; i < numLadders; i++) {
                ...
                cin >> start >> end;
                strategy->addLadderPositions(start, end);
            }
