...
//discount strategy (strategy design pattern)
...
class PercentageDiscountStrategy : public DiscountStrategy {
    ...
    public:
    PercentageDiscountStrategy(double pt){
        percent = pt;
    }
...
class PercentageWithCapStrategy : public DiscountStrategy {
    ...
    public:
    PercentageWithCapStrategy(double pt, double cap){
        percent = pt;
        amount = cap;
    }

    double calculate(double baseAmount) override {
        double t = (percent/100) * baseAmount;
        if(t>amount) return amount;
        return t;
    }
};

//DiscountStrategyManager or Discount Factory (singleton)
...
//model for prduct and cart assuming they are fetched from the user
class Product {
    private:
    string name ;
    string category;
